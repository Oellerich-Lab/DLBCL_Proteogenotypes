...
```{r}
meta_scPG4 <- read_delim("~/sc_DLBCL/Staudt_scData/20230403.metadata.44dlbcl.tsv", 
...
folder_path <- "/projects/dlbcl-sc/StaudtLab_scRNA_primary"
...
pg4_pred <- read_excel("20230403_ProtGen_Samples_Mapped.xlsx")  %>%
  filter(ProtGen_SG == "4")
...
projPG4$pg4celltype <-
  (
    getCellColData(projPG4) %>%
      ...
      #     Sample %in% pg4_pred$Frankfurt_ID &
...
staudt_sig_sets <- readRDS("20230419_staudt_gene_sets.rds")
...
seurat_dlcbl <- readRDS("seurat.integrated.rds")
...
```{r}
saveRDS(seurat_ob, "seurat_ob_RNA_ATAC.rds")
```
